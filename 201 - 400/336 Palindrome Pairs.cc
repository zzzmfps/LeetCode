        ...
        for (int i = 0; i < size1; ++i)
        {
            ...
            for (int j = 0; j < size2 + 1; ++j)
            {
                if (is_equal(i, 0, j))
                {
                    ...
                    if (fus != words[i] && umap.find(fus) != umap.end())
                        res.push_back(vector<int>{umap[fus], i});
                }
                if (j != size2 && is_equal(i, j, size2))
                {
                    auto erp = string(words[i].rend() - j, words[i].rend());
                    if (umap.find(erp) != umap.end())
                        res.push_back(vector<int>{i, umap[erp]});
                }
            }
        }
        return res;
